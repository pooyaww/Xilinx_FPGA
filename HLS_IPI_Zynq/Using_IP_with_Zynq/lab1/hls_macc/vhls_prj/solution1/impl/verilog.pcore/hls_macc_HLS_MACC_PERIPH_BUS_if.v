...
#(parameter
    ...
    input  wire [31:0]               O_accum,
    input  wire                      O_accum_ap_vld,
    output wire [0:0]                I_accum_clr,
...
localparam
    ...
    ADDR_B_CTRL           = 6'h18,
    ADDR_B_DATA_0         = 6'h1c,
    ADDR_ACCUM_CTRL       = 6'h20,
    ADDR_ACCUM_DATA_0     = 6'h24,
    ADDR_ACCUM_CLR_CTRL   = 6'h28,
    ADDR_ACCUM_CLR_DATA_0 = 6'h2c;
...
assign I_a         = _a;
assign I_b         = _b;
assign _accum      = O_accum;
assign I_accum_clr = _accum_clr;
...
always @(posedge ACLK) begin
    if (~ARESETN)
        _accum_ap_vld <= 1'b0;
    else if (O_accum_ap_vld)
        _accum_ap_vld <= 1'b1;
    else if (ar_hs && raddr == ADDR_ACCUM_CTRL)
        _accum_ap_vld <= 1'b0; // clear on read
end

// _accum_clr[0:0]
always @(posedge ACLK) begin
    if (w_hs && waddr == ADDR_ACCUM_CLR_DATA_0)
        _accum_clr[0:0] <= (WDATA[31:0] & wmask) | (_accum_clr[0:0] & ~wmask);
end
