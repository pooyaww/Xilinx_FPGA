...
parameter AUTOTB_TRANSACTION_NUM = 4433;
...
reg AESL_REG_accum_ap_vld = 0;
// The signal of port accum
reg [31: 0] AESL_REG_accum = 0;
always @(posedge AESL_clock)
begin
    if(AESL_reset)
        AESL_REG_accum = 0; 
    else if(accum_ap_vld) begin
        AESL_REG_accum <= accum;
        AESL_REG_accum_ap_vld <= 1;
    end
end 

initial begin : write_file_process_accum
    integer fp;
    integer fp_size;
    integer err;
    integer ret;
    ...
    while (transaction_idx != AUTOTB_TRANSACTION_NUM) begin
        ...
        if(AESL_REG_accum_ap_vld)  begin
          $fdisplay(fp,"0x%x", AESL_REG_accum);
        AESL_REG_accum_ap_vld = 0;
        end
...
// The signal of port accum_clr
reg [0: 0] AESL_REG_accum_clr = 0;
assign accum_clr = AESL_REG_accum_clr;
initial begin : read_file_process_accum_clr
    integer fp;
    integer err;
    integer ret;
    integer proc_rand;
    ...
    transaction_idx = 0;
    wait(AESL_reset === 0);
    fp = $fopen(`AUTOTB_TVIN_accum_clr,"r");
    if(fp == 0) begin       // Failed to open file
        $display("Failed to open file \"%s\"!", `AUTOTB_TVIN_accum_clr);
        $display("ERROR: Simulation using HLS TB failed.");
        $finish;
    end
    read_token(fp, token);
    ...
    while (token != "[[[/runtime]]]") begin
        ...
        if(token != "[[/transaction]]") begin
            ret = $sscanf(token, "0x%x", AESL_REG_accum_clr);
              if (ret != 1) begin
                  $display("Failed to parse token!");
                $display("ERROR: Simulation using HLS TB failed.");
                  $finish;
...
initial begin : generate_AESL_ready_cnt_proc
    ...
    initial begin : finish_simulation
        ...
        @ (posedge AESL_clock);
        @ (posedge AESL_clock);
    fp1 = $fopen("./rtl.hls_macc.autotvout_accum.dat", "r");
    fp2 = $fopen("./impl_rtl.hls_macc.autotvout_accum.dat", "r");
    if(fp1 == 0)        // Failed to open file
        $display("Failed to open file \"./rtl.hls_macc.autotvout_accum.dat\"!");
    else if(fp2 == 0)
        $display("Failed to open file \"./impl_rtl.hls_macc.autotvout_accum.dat\"!");
    else begin
        $display("Comparing rtl.hls_macc.autotvout_accum.dat with impl_rtl.hls_macc.autotvout_accum.dat");
        post_check(fp1, fp2);
    end
    $fclose(fp1);
    $fclose(fp2);
